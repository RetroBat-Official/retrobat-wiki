...
Les ROMs de jeux doivent être au format .zip et contenir dans le fichier compressé les fichiers .bin.

Le fichier `ti99_cart.xml` situé dans le répertoire `\bios\mame\hash` permet de vérifier le bon nommage des roms et de leur contenu et de déterminer si vous possédez les ROMs de jeux compatibles.
